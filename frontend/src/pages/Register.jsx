  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-between bg-[#2e2b8f] dark:bg-slate-950 p-4 md:p-6 font-sans transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl grid md:grid-cols-12 bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-white/10 dark:border-slate-800 transition-colors duration-300 relative z-10 my-auto">
        ...
        {/* Form Container */}
        <div className="md:col-span-7 p-6 md:p-8 flex flex-col justify-center bg-white dark:bg-slate-900 transition-colors duration-300">
          <div className="mb-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-xs font-medium transition-all min-h-[38px] ${
                      formData.role === value
                        ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40 dark:border-indigo-400 text-indigo-700 dark:text-indigo-300 ring-2 ring-indigo-500/20'
                  ...
                  >
                    ...
                    {formData.role === value && <CheckCircle2 size={12} className="ml-auto shrink-0 text-indigo-500" />}
                  </button>
                ))}
              </div>

              {/* Permission badges for selected role */}
              <div className="mt-2">
                <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase mb-1">
                  {formData.role.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())} — Access to:
                </p>
                <div className="flex flex-wrap gap-1">
                  ...
                  )[formData.role].map(page => (
                    <span key={page} className="px-1.5 py-0.5 text-[9px] font-medium bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 rounded border border-indigo-200 dark:border-indigo-800/50">
                      {page}
                    </span>
                  ))}
                </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-1 py-2.5 px-4 bg-[#2e2b8f] hover:bg-[#252275] active:bg-[#1d1a5c] dark:bg-indigo-600 dark:hover:bg-indigo-500 dark:active:bg-indigo-700 text-white font-semibold rounded-xl text-sm shadow-md shadow-[#2e2b8f]/20 dark:shadow-none transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
