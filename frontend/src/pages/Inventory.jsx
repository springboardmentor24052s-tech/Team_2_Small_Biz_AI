...
export default function Inventory() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await api.post(
        '/inventory/products/upload-csv',
        data,
        { headers: { 'Content-Type': 'multipart/form-data' } }
      )

      setUploadMsg(
        `Uploaded: ${res.data.products_created} created, ` +
        `${res.data.products_updated} updated, ` +
        `${res.data.rows_skipped} skipped.`
      )
  ...
  const filteredProducts = products.filter((p) => {
    ...
    const reorderLevel = p.inventory?.reorder_level ?? 0
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 pr-4 text-slate-500">
                        {p.inventory?.warehouse_location || '—'}
                      </td>
