    ...
    <html>
      <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6;">
        <div style="max-width: 500px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 12px; padding: 24px;">
          ...
          <div style="background-color: #f4f4f9; text-align: center;
                      font-size: 28px; font-weight: bold; letter-spacing: 4px;
                      padding: 12px; margin: 20px 0; border-radius: 8px;
                      color: #2e2b8f;">
            {otp_code}
          </div>

          <p style="font-size: 12px; color: #777;">
            This code is valid for 15 minutes. If you did not request this,
...
def register(
    ...
    role = (
        ...
        .filter(models.Role.role_name == "business_owner")
        .first()
    )
...
def send_otp(
    ...
    user.reset_otp_expiry = (
        dt.datetime.utcnow() + dt.timedelta(minutes=15)
    )
    ...
    return {
        "message": f"OTP code sent to {payload.email}."
    }
