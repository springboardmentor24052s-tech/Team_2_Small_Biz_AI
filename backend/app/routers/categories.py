...
def list_categories(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return db.query(models.Category).filter(models.Category.business_id == current_user.business_id).order_by(models.Category.category_name).all()
...
def delete_category(
    ...
    category = db.query(models.Category).filter(models.Category.id == category_id, models.Category.business_id == current_user.business_id).first()
    if not category:
