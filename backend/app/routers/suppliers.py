...
def list_suppliers(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return db.query(models.Supplier).filter(models.Supplier.business_id == current_user.business_id).order_by(models.Supplier.supplier_name).all()
...
def delete_supplier(
    ...
    supplier = db.query(models.Supplier).filter(models.Supplier.id == supplier_id, models.Supplier.business_id == current_user.business_id).first()
    if not supplier:
